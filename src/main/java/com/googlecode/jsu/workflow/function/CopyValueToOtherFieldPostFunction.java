 ...
 * Adapted from  CopyValueFromOtherFieldPostFunction by R.Popov to update the *to* issue (this or parent)
 *
 * This function copies the value to a field to another one.
 */
public class CopyValueToOtherFieldPostFunction extends AbstractPreserveChangesPostFunction {
    private final WorkflowUtils workflowUtils;
    private final I18nHelper.BeanFactory beanFactory;

    public CopyValueToOtherFieldPostFunction(WorkflowUtils workflowUtils, I18nHelper.BeanFactory beanFactory) {
    ...
    protected void executeFunction(
        ...
        try {
            ...
            if("parent".equals(copyType)) {
                sourceIssue = issue;
                destIssue = (MutableIssue) issue.getParentObject();

                if(destIssue==null) {
                    log.debug("Issue (" + sourceIssue.getKey() + ") has no parent, wont do anything.");
                    return;
                }
            } else {
